...
describe 'tempest' do
  shared_examples 'tempest' do
    ...
    context 'with image_ref parameters' do
      let :params do
        { :image_ref     => 'image-uuid-1',
          :image_ref_alt => 'image-uuid-2' }
      end

      it 'configures image_ref' do
        is_expected.to contain_tempest_config('compute/image_ref').with_value('image-uuid-1')
      end
    end
    ...
    context 'with public_network_id parameter' do
      ...
      it 'configures public_network_id' do
        is_expected.to contain_tempest_config('network/public_network_id').with_value('net-uuid')
      end
    end
